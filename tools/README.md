...
None of this is part of the driver or the RPM. These programs gave the
protocol. They stay here for two reasons. A change to one of them is much
faster than a full `fprintd` cycle. And they gave the measurements that the
documentation of the driver holds.
...
- **`ft9201-capture.c`** — arms the sensor and grabs N frames to `linux-frameNN.pgm`. It uses the three
  gates of the driver: request `0x43`, register `0x30` = `0xbb` and register
  `0x1d` = `0xa0`. It also makes the same `dd dd` check on the header. It
  prints the statistics of each frame.
...
- **`ft9201-matcher.c`** — measures the matcher of the driver on saved
  ...
  `FT9201_MATCH_THRESHOLD`.** It needs no hardware.
  ...
  3. the error rates against the threshold, for the operation of the driver. That
  operation scores each frame against a template of the other frames of its
  group. It uses the second-best rule of `ft9201_match_template`,
  ...
  **Collect the frames as the sensor is used.** Put the finger down the
  same way each time. Do not change the position on purpose. The sensor is 3 x 4 mm. A changed position records a different area of the
  same finger, and two different areas do not correlate.
...
- **`mosaic.py`** — measures how far apart consecutive presses land, by phase
  ...
  area. Note that it uses the opposite sign for a shift; the correlation values
  are what matter.
...
A watch on the registers for the idle stall, which only reads. It reads
`0x01`, `0x1d`, `0x1e`, `0x1f`, `0x20`, `0x22`, `0x23`, `0x30`, `0x41` and
request `0x43` once a second. It prints a line only when a value changes,
and a heartbeat every 20 s. It writes nothing, thus it cannot disturb a
sensor that the driver armed.
...
Makes the idle stall outside the driver, and watches both finger signals
through it. It polls request `0x43` and register `0x1d` together every 80
ms, and it makes the renewal of the vendor after each 1000 idle polls. It
prints a line when one of the two signals changes. It also marks the
condition that matters: `0x43` = `00` while `0x1d` = `a0`. That condition
shows that the AFE still sees the finger, and that only the interrupt port
went quiet. If the tool prints that condition, the driver can poll both
signals, and it then needs no power cycle.
...
- **`rotation-test.c`** — measures the effect of a turn on the matcher of
  the driver. It turns each frame, and it then scores the turned frame against the
  original frame. It uses the same include file as
...
- **`fpn-experiment.c`** — measures a correction of the fixed pattern of
  ...
  numbers. It needs no frame of the empty sensor: the tool takes the
...
Use one bell to say "the sensor took that press, press again". The enrolment test
